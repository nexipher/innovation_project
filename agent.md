...
### 4.1 已下载并上传的本地数据清单
数据位于 `/dataset` 中，Claude Code 在编写数据加载器（Data Loader）和测试用例时应直接挂载以下资源：
1.  **Real** (`dataset/Real/`)：真实照片。
2.  **GenImage_Test** (`dataset/GenImage_Test`)：内含 Wukong、Midjourney 等八个模型的伪造照片。
...
## 💡 Claude Code 执行时的工作流记忆口诀（可供 Agent 自检）

> 1. **看需求**：读 `{project}_tasks.md`，不乱改。
> 2. **定计划**：写 `plan.md`，拆解到 `X.Y` 小节；有变更，先追加。
> 3. **小步走**：每做完一个 `X.Y` 小节，跑通测试，立个 `commit`。
> 4. **记日志**：每完结一个 `X.Y` 二级小节，追加 `claude_operation_log.md`。
> 5. **结大章**：每搞定一个 `X` 大节，更新 `README.md`。
> 6. **动显卡**：凡是加载权重、用 GPU，必须停下问用户“确认使用 GPU”。
