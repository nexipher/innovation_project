...
- **G3 改进（尚未在 GPU 层复测）**：①整图度量（G3-a）②EvidenceRectifier 统一方向权威，同 token 内矛盾结构性消除（G3-b）③停止策略 v2 以校准后验决策：回放 1191 条真实会话，v1 标注 802 条准确率 0.414，v2 标注 367 条准确率 0.534、冲突样本 74/74 弃权（v1 曾给其中 14 条定标签）（G3-c）④实验配置指纹，配置变更不再静默复用旧结果（G3-d）。**n=120 GPU 复核待授权**。
