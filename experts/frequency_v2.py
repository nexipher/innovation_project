...
class FrequencyExpertV2(BaseExpert):
    ...
    counter_explanation = (
        ...
        "G2 校准显示 v2 为弱信号（格式配平后 AUROC≈0.56-0.63），只应作为辅助证据。"
    )
    ...
    def render_artifacts(self, img_patch: np.ndarray) -> dict:
        """Render the log-power spectrum used for the multi-scale decision."""
        if img_patch.ndim == 3:
            gray = (
        ...
        return {"frequency_spectrum_v2": cv2.applyColorMap(normalized, cv2.COLORMAP_INFERNO)}
